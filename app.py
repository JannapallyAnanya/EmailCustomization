...
import google.auth
from google_auth_oauthlib.flow import Flow  # Import the OAuth 2.0 flow
import oauthlib
...
app.config['MYSQL_PASSWORD'] = 'Seq@123$'
app.config['MYSQL_DB'] = 'email_customizer_db'
...
CLIENT_SECRETS_FILE = "client_secrets.json"  # Download from Google Developer Console
SCOPES = [
...
def email_customization():
    ...
    if request.method == 'POST':
        ...
        if "Emails sent successfully!" in result:
            ...
            return redirect(url_for('dashboard'))  # Redirect to dashboard after email is sent
        else:
            flash(result, 'danger')
    print('email custom')
